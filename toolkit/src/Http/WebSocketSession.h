...
public:
    ...
    SessionTypeImp(const toolkit::Parser &header, const toolkit::HttpSession &parent, const toolkit::Socket::Ptr &pSock) :
            SessionType(pSock) {}
...
public:
    ...
    toolkit::Session::Ptr operator()(const toolkit::Parser &header, const toolkit::HttpSession &parent, const toolkit::Socket::Ptr &pSock){
...
*/
template<typename Creator, typename HttpSessionType = toolkit::HttpSession, toolkit::WebSocketHeader::Type DataType = toolkit::WebSocketHeader::TEXT>
...
public:
    ...
    void onManager() override{
        ...
        if (_recv_ticker.elapsedTime() > 30 * 1000) {
            ...
            // 没收到回复，每10秒发送次ping 包
            toolkit::WebSocketHeader header;
            header._fin = true;
            header._reserved = 0;
            header._opcode = toolkit::WebSocketHeader::PING;
...
protected:
     ...
     */
    bool onWebSocketConnect(const toolkit::Parser &header) override{
     ...
     * 开始收到一个webSocket数据包
     */
    void onWebSocketDecodeHeader(const toolkit::WebSocketHeader &packet) override{
        //新包，原来的包残余数据清空掉
     ...
     * 收到websocket数据包负载
     */
    void onWebSocketDecodePayload(const toolkit::WebSocketHeader &packet, const uint8_t *ptr, size_t len, size_t recved) override {
     ...
     * @param header 数据包包头
     */
    void onWebSocketDecodeComplete(const toolkit::WebSocketHeader &header_in) override {
        auto header = const_cast<toolkit::WebSocketHeader&>(header_in);
        auto  flag = header._mask_flag;
        header._mask_flag = false;
        _recv_ticker.resetTime();
        switch (header._opcode){
            case toolkit::WebSocketHeader::CLOSE:{
                HttpSessionType::encode(header,nullptr);
                HttpSessionType::shutdown(toolkit::SockException(toolkit::Err_shutdown, "recv close request from client"));
                break;
            }
            
            case toolkit::WebSocketHeader::PING:{
                header._opcode = toolkit::WebSocketHeader::PONG;
                HttpSessionType::encode(header,std::make_shared<toolkit::BufferString>(_payload_section));
                break;
            }
            
            case toolkit::WebSocketHeader::CONTINUATION:
            case toolkit::WebSocketHeader::TEXT:
            case toolkit::WebSocketHeader::BINARY:{
                if (!header._fin) {
                ...
                if (_payload_cache.empty()) {
                    //这个包是唯一个分片
                    _session->onRecv(std::make_shared<toolkit::WebSocketBuffer>(header._opcode, header._fin, std::move(_payload_section)));
                    break;
                }

                //这个包由多个分片组成
                _payload_cache.append(std::move(_payload_section));
                _session->onRecv(std::make_shared<toolkit::WebSocketBuffer>(header._opcode, header._fin, std::move(_payload_cache)));
...
template<typename SessionType,typename HttpSessionType = toolkit::HttpSession, toolkit::WebSocketHeader::Type DataType = toolkit::WebSocketHeader::TEXT>
